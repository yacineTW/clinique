...
require_once("C:/xampp/htdocs/web2A/Controllers/mailing.php");

$DoctorController = new DoctorController();
$historique = $DoctorController->getDossierByPatientId($_GET['patient_id']);
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $historique = $_POST['historique'];
    if ($DoctorController->HistoriqueExists($_GET['patient_id'])) {
        sendMail('useremail@email.com', 'Votre dossier médical à été mis a jour', '<h1>Hello</h1><p>Your medical record has been updated</p>');

        echo "historique is not empty";
        // Update existing historique

        $DoctorController->updateHistorique($_GET['patient_id'], $historique);
    } else {
        echo "historique doesnt exist";
        // Create new historique
        sendMail('useremail@email.com', 'Votre dossier médical à été crée', '<h1>Hello</h1><p>Your medical record has been created</p>');

        $DoctorController->addHistorique($_GET['patient_id'], $historique);
    }
   // header('Location: DossierMedical.php?id=' . $_GET['id']);
    //exit();
}
?>
...
<head>
    ...
    <title>Dashmix - Bootstrap 5 Doctor Template &amp; UI Framework</title>
                                                ...
                                                <div class="block mb-0">
                                                    ...
                                                    <div class="block-content block-content-full">
                                                        <div class="mb-4">
                                                            ...
                                                            <input type="text" readonly="" class="form-control" id="so-profile-username-static" value="Doctor">
                                                        </div>
                                                        <div class="mb-4">
                ...
                <div class="block-content">
                    <div class="row justify-content-center">
                        <div class="col-md-10 col-lg-8">
                            <form action="DossierMedical.php?patient_id=<?php echo $_GET['patient_id']?>" method="POST" >

                                <div class="mb-4">
                                    <label class="form-label" for="historique">Historique de patient </label>
                                    <textarea class="form-control" id="historique" name="historique" rows="4" placeholder="Patient History..">
  <?php
  if (isset($historique['historique']))
    echo $historique['historique'];
  else if(isset($historique))
      echo $historique;
  else
      echo "";
  ?>                                    </textarea>
                                ...
                                <div class="mb-4">
                                    <button type="submit" class="btn btn-alt-primary">Update Patient History</button>
                                </div>
